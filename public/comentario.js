//organizar a pasta public com seus respectivos assets

// Projeto Customy:

// Páginas criadas:

// - Login [ok]
// - Signup [ok]
// - Customers [ok]
// - Notes [ok]


// ---------------------------------

// Próximas tarefas:


// 01 - Criar formulários de criação e edição de notas.

/* 02 - Implementar modal de confirmação de exclusão de cliente
baseado no projeto Tasklist e replicar a mesma lógica para as notas. */

// 03 - Fazer o roteamento das páginas que faltam no App.jsx.

// - criar botão de voltar/cancelar

// - criar mecanismo de recuperação de senha;

// - implementar modal de confirmação de exclusão de item;

// - carregar página de edição de cliente com os dados do cliente pré-preenchidos;

// - implementar mecanismo de busca por nome na página de clientes;





